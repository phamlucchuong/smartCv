...
run-job:
	set -a; [ -f $(BACKEND)/.env ] && . $(BACKEND)/.env; set +a; \
	cd $(BACKEND)/job_service && $(MVN) spring-boot:run

run-application:
	set -a; [ -f $(BACKEND)/.env ] && . $(BACKEND)/.env; set +a; \
	cd $(BACKEND)/application_service && $(MVN) spring-boot:run

run-ai:
	set -a; [ -f $(BACKEND)/.env ] && . $(BACKEND)/.env; set +a; \
	cd $(BACKEND)/ai_engine_service && $(MVN) spring-boot:run
...
run-backend:
	...
	@set -a; [ -f $(BACKEND)/.env ] && . $(BACKEND)/.env; set +a; \
	(cd $(BACKEND)/user-service        && $(MVN) spring-boot:run 2>&1 | tee ../logs/user-service.log) & \
	(cd $(BACKEND)/api-gateway         && $(MVN) spring-boot:run 2>&1 | tee ../logs/api-gateway.log) & \
	(cd $(BACKEND)/job_service         && $(MVN) spring-boot:run 2>&1 | tee ../logs/job-service.log) & \
	(cd $(BACKEND)/application_service && $(MVN) spring-boot:run 2>&1 | tee ../logs/application-service.log) & \
	(cd $(BACKEND)/ai_engine_service   && $(MVN) spring-boot:run 2>&1 | tee ../logs/ai-engine-service.log) & \
...
migrate-user:
	set -a; [ -f $(BACKEND)/.env ] && . $(BACKEND)/.env; set +a; \
	timeout 45s sh -c 'cd $(BACKEND)/user-service && $(MVN) spring-boot:run \
	  -Dspring-boot.run.arguments="--server.port=0 --spring.main.web-application-type=none"' \
	|| [ $$? -eq 124 ]

migrate-job:
	set -a; [ -f $(BACKEND)/.env ] && . $(BACKEND)/.env; set +a; \
	timeout 45s sh -c 'cd $(BACKEND)/job_service && $(MVN) spring-boot:run \
	  -Dspring-boot.run.arguments="--server.port=0 --spring.main.web-application-type=none \
